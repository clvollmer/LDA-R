...
#Professor Azencott - Machine Learning - Project #1
#computes the covariance Matrix for entire set of data, banknotes

library(ggplot2)
library(kernlab)
library(gridExtra)

###Performa  t-test on the attributes###
ttest = data.frame()
for (a in 1:4)
{
  ttest[1,a]=t.test(training[,a]~training[,5], conf.level = .99)
}
colnames(ttest) <- c("Variance WTI", "Skewness WTI", "Kurtosis WTI", "Entropy WTI")
rownames(ttest) <- ("T-Test")
View(ttest)


cov_matrix <- data.frame() 
cov_matrix <- cov(training[,1:4]) #preallocate covariance matrix

names(cov_matrix) <- c("Variance WTI", "Skewness WTI", "Curtosis WTI", "Entropy WTI")
row.names(cov_matrix) <- t(c("Variance WTI", "Skewness WTI", "Curtosis WTI", "Entropy WTI")) #easy way to name symmetric matrices
...
run_sum = data.frame(`Eigenvalue` = training_eigen[,1],`Running Sum` = 0, Threshold = character(4),stringsAsFactors=FALSE)
count = 0
...
for(i in 1:4)
{
  ...
  count = run_sum[i,2]
  run_sum[i,3] = paste(round(100*count/sum(training_eigen), 4),'% Threshold Met')
}


y=data.frame(1:4)
train_gauss_eigen = cbind(y,training_eigen,run_sum[1:4,2])
...
q=ggplot(data = train_gauss_eigen,aes(x=train_gauss_eigen[,1], y=train_gauss_eigen[,2])) + geom_point(size = 3)+ 
  ...
  geom_text(x=train_gauss_eigen[2,1], y=train_gauss_eigen[2,2], label = run_sum[2,3],
            hjust = -.1, vjust = .5, size = 4, color = 'red')+
  geom_text(x=train_gauss_eigen[3,1], y=train_gauss_eigen[3,2], label = run_sum[3,3],
            hjust = 1.1, vjust = .5, size = 4, color = 'red')+
  geom_text(x=train_gauss_eigen[4,1], y=train_gauss_eigen[4,2], label = run_sum[4,3],
            hjust = 1.15, vjust = .5, size = 4, color = 'red')
...
qq=ggplot(train_gauss_eigen) + geom_line(aes(train_gauss_eigen[,1], train_gauss_eigen[,3]), size = 1) + 
  ...
  geom_vline(aes(xintercept = 3, linetype = 2), color = 'red', size = .5, linetype = 2)+
  geom_text(x=3, y=train_gauss_eigen[3,3], label = run_sum[3,3],
...
###!!!THE BELOW CODE IS DEFUNCT A BIT. CHANGED HOW DATAFRAME WAS ORGANIZED!!!###
###!!!I did not need this code, no need to look at gramian, just covariance matrix!!!###
...
rbf <- rbfdot(sigma = .1)
train_gauss = kernelMatrix(rbf, as.matrix(training[,1:4]))
